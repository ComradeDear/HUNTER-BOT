# Rewritten by DEAR COMRADE
import io
from  userbot import CMD_LIST, ALIVE_NAME, bot as hunt
...
@hunt.on(hunter_cmd(pattern="help ?(.*)"))
async def cmd_list(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        hunterusername = Var.TG_BOT_USER_NAME_BF_HER
        input_str = event.pattern_match.group(1)
        if hunterusername is None or input_str == "text":
            string = ""
            for i in CMD_LIST:
                string += "ℹ️ " + i + "\n"
           ...
           hunt_help_strin = """**HUNTERBOT Is Here With The Detailed Help For CMDs** 😉😉 !\n If Faced Any Bug Please Give The Feed Back at [HUNTERBOT Support](https://t.me/hunterbot_support):"""
