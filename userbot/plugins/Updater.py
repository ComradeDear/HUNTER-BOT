...
import asyncio
from userbot.events import hunter_cmd
config=Config
...
@borg.on(hunter_cmd(pattern="update ?(.*)"))
async def upstream(ups):
    ...
    if force_updateme:
        ...
        await ups.edit('`Updating your`HUNTERBOT 𝐔𝐬𝐞𝐫𝐛𝐨𝐭 `please wait for a while`')
        await asyncio.sleep(4)
        await ups.edit('Updated Sur✨\nRestarting it please have patience and enjoy botless life for a while\nIncase restart structs join [HUNTERBOT 𝐒𝐮𝐩𝐩𝐨𝐫𝐭](https://t.me/hunterbot_support) ')
    ...
    if config.HEROKU_API_KEY is not None:
        ...
        await ups.edit('`Sync Verified Successfully 🙂🙂\n'
                       'Restarting, please wait a min ,then type .alive to check if I alive !! Else Go to [HUNTERBOT 𝐒𝐮𝐩𝐩𝐨𝐫𝐭](https://t.me/hunterbot_support)`')
    else:
