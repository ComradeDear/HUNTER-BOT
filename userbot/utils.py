...
def hunter_cmd(pattern=None, command=None, **args):
    args["func"] = lambda e: e.via_bot_id is None
    stack = inspect.stack()
    previous_stack_frame = stack[1]
    file_test = Path(previous_stack_frame.filename)
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[Here](https://t.me/hunterbot_support)"
            ...
            ftext += "--------BEGIN HUNTERBOT USERBOT TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        spec.loader.exec_module(mod)
        sys.modules[
            "userbot.plugins.assistant" + "Initialising HUNTERBOT" + shortname
        ] = mod
        sedprint.info("Hunterbot Has imported " + shortname)
...
def load_assistant(shortname):
    if shortname.startswith("__"):
        ...
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        print("Initialising HUNTERBOT.")
        print("Hunterbot - Imported " + shortname)
    else:
        import importlib
        import sys
        from pathlib import Path
        ...
        print("Hunterbot Has imported " + shortname)
